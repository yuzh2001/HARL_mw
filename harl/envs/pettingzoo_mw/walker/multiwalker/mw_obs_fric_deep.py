...
class BipedalWalker(Agent):
    ...
    def get_observation(self):
        ...
        frc = self.env.terrain[0].fixtures[0].friction
        delta_frc = -2.4 if frc == 0.1 else 2.4
        # \delta a_x = \delta \mu(F\cos \theta+Mg)
        F1 = (
            self.joints[1].motorTorque
            - self.env.package.mass
            * LEG_H
            * np.sin(self.joints[0].angle + 3.14 - self.joints[1].angle)
        ) / (LEG_H * np.sin(3.14 - self.joints[1].angle))
        F2 = (
            self.joints[3].motorTorque
            - self.env.package.mass
            * LEG_H
            * np.sin(self.joints[2].angle + 3.14 - self.joints[3].angle)
        ) / (LEG_H * np.sin(3.14 - self.joints[3].angle))
        delta_a = delta_frc * (self.env.package.mass * 9.8) + delta_frc * (
            F1 * np.cos(3.14 - self.joints[1].angle)
            + F2 * np.cos(3.14 - self.joints[3].angle)
        )
